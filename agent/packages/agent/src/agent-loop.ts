		...
		try {
			...
			if (prefetched.length > 0) {
				...
				const urgency = isVeryTight
					? `\n\n*** ULTRA-TIGHT TIMEOUT: ${timeoutSec}s. Minimize discovery — you already have file bodies above. Plan briefly, then \`write\`/\`edit\`. Empty diff guarantees a loss. ***`
					: timeoutSec > 0
						? `\n\n*** YOUR TIMEOUT: ${timeoutSec}s (King has 300s). YOU ARE THE UNDERDOG. After a short read/grep if needed, EDIT. ***`
						: `\n\n*** After Reading→Thinking on what you have above, start Editing (\`write\`/\`edit\`). Empty diff loses. ***`;
	...
	while (true) {
		...
		while (hasMoreToolCalls || pendingMessages.length > 0) {
			...
			if (hasMoreToolCalls) {
				...
				if (workPhase === "search") {
					for (const tr of toolResults) {
						if (tr.toolName === "bash" && !tr.isError) {
							...
							if (paths.length > 0) {
								...
								pendingMessages.push({
									role: "user",
									content: [{ type: "text", text: `Located ${foundFiles.length} candidate files (Reading phase). \`read\` each file you may modify and follow important imports/deps. **Editing phase:** **re-read** right before each \`edit\`/\`write\`. Targets:\n${[...foundFiles].slice(0, 10).map((p: string) => `- ${p}`).join("\n")}` }],
									timestamp: Date.now(),
								});
							}
						}
					...
					if (workPhase !== "apply") {
						...
						if (absorbedFiles.size >= absorbLimit && workPhase === "absorb" && pendingMessages.length === 0) {
							...
							pendingMessages.push({
								role: "user",
								content: [{ type: "text", text: `${absorbedFiles.size} files absorbed (Reading phase). **Editing phase:** **re-read** each file you will change (or the slice you need) immediately before \`edit\`/\`write\`. Then apply changes. **Re-evaluation:** check criteria; loop read→edit if gaps remain. Cover acceptance criteria first; open additional files only when a criterion, named path, or required wiring requires it.` }],
								timestamp: Date.now(),
							});
						}
					}
				...
				const now = Date.now();
				// Allow more re-reads after edits (re-evaluation, re-read-before-edit); tighten only during long pre-edit exploration.
				const rereadNudgeThreshold = hasProducedEdit ? 6 : 3;
				if (now - lastRereadNudgeAt >= 5_000 && pendingMessages.length === 0) {
					for (const [rp, cnt] of pathReadCounts) {
						if (cnt >= rereadNudgeThreshold) {
							...
							pendingMessages.push({
								...
								content: [
									{
										type: "text",
										text: `You have read \`${rp}\` ${cnt} times without broadening work. Unless each read is **immediately before an \`edit\`**, **after a failed edit**, or **re-evaluation**, switch focus: ${others.length > 0 ? `edit or read unedited targets ${others.slice(0, 5).map((f: string) => `\`${f}\``).join(", ")}.` : "Apply \`edit\`/\`write\` or read a different file."}`,
									},
								],
								timestamp: Date.now(),
							});
		...
		if (!reviewPassDone && hasProducedEdit && reviewElapsed < 60_000) {
			...
			const hint = uneditedTargets.length > 0
				? `Unedited discovered files: ${uneditedTargets.slice(0, 5).map((f: string) => `\`${f}\``).join(", ")}. **Re-evaluation:** loop Reading→Thinking→Editing until addressed or confirm N/A.`
				: `**Re-evaluation:** Walk acceptance criteria; \`grep\` for any exact old strings/labels the task named. If gaps remain, **re-read** and **edit**; if fully satisfied, reply "done".`;
			pendingMessages = [{
				role: "user",
				content: [{ type: "text", text: `REVIEW (Re-evaluation pass): You edited ${pathEditCounts.size} file(s): ${[...pathEditCounts.keys()].slice(0, 8).join(", ")}. ${hint}` }],
				timestamp: Date.now(),
			}];
			continue;
		}
