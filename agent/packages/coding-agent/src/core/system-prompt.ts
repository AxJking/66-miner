...
function buildTaskDiscoverySection(taskText: string, cwd: string): string {
	try {
		...
		sections.push(
			"READING PHASE — DISCOVERY ORDER: (1) Derive keywords from the task (named paths, symbols, quoted strings, criteria). (2) Run `grep` / rg (or bash `grep -r`) for exact phrases from the task and acceptance bullets before shallow `find`/directory listing. (3) Prefer paths that hit multiple phrases; break ties for explicitly named files. (4) Use `find`/`ls` only for gaps. Then `read` candidates and dependencies before the thinking/editing loop.",
		);
		...
		sections.push("\nAnti-stall: after enough discovery reads (≈2 in small-task mode, ≈3 in multi-file), move from Reading into Thinking+Editing — but still **re-read immediately before** each mutation.");
...
const TAU_SCORING_PREAMBLE_FOR_MAIN_BRANCH = `## Work cycle (repeat until the task is done)

Follow this loop every time; do not skip ahead without evidence from the repo.

1. **Reading** — Extract **keywords** from the task (symbols, paths, strings, criteria phrases). Use \`grep\` / \`find\` (or \`bash\`) to locate relevant files. \`read\` files you may change and follow **imports / types / configs** when the task implies behavior there.
2. **Thinking** — From what you read plus the task, decide exactly what to change, in what order, and which files must be touched for each acceptance criterion.
3. **Editing** — **Re-read** each target (or the slice you need) **immediately before** \`edit\` / \`write\` so \`oldText\` matches disk. Change only what the task requires.
4. **Re-evaluation** — Check whether the task is **fully** satisfied. If criteria or named behavior are **missing**, go back to Reading/Thinking. If an edit was **wrong**, **re-read** and fix—never patch from stale memory. If everything required is satisfied, stop mutating.

Loop: Reading → Thinking → Editing → Re-evaluation → (repeat if needed).
...
Flow: read to understand -> plan -> **re-read right before each edit** -> apply copy/UI edits in top-to-bottom order -> **re-evaluate** -> only then consider other files.
...
- **Reading phase:** grep/find using task **keywords** first; then \`read\` hits and necessary dependencies.
...
- Adaptive cutoff: in Mode A (small-task), after 2 discovery/search steps make the first valid minimal edit; in Mode B (multi-file), use 3 steps; in Mode C, after 2 grep/read steps start editing the concentrated file — always **re-read** the target right before each \`edit\`.
...
Before stopping, run the **re-evaluation** pass mentally (and with \`grep\` if the task named exact strings to remove):
...
- Completeness cross-check: walk through each acceptance criterion one-by-one and verify you have a corresponding edit. If any criterion is unaddressed, **loop** (read → think → edit) again.
...
When this re-evaluation passes, stop mutating immediately (no extra churn).
...
## Work cycle (repeat until the task is done)

1. **Reading** — Derive **keywords** from the task; use \`grep\` / \`find\` to locate files; \`read\` targets and needed dependencies.
2. **Thinking** — Plan changes from evidence in the repo plus the task.
